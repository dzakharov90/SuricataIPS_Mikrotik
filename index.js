...
try {
    tail.on("line", data => {
        if (data.match(/[A-Za-z]+ [\d]+ [\d]+\:[\d]+\:[\d]+ [A-Za-z]+ [A-Za-z]+\[[\d]+]\: \[[\d]+\:([\d]+)\:[\d]+\] ([A-Za-z0-9 ]+) \[[A-Za-z0-9: ]+\] \[[A-Za-z0-9: ]+\] \{[A-Za-z0-9]+\} ([\d]+\.[\d]+\.[\d]+\.[\d]+)\:[\d]+ \-\> [\d]+\.[\d]+\.[\d]+\.[\d]+\:[\d]+/gm)) {
            console.log(data);
            const RuleMatch = /[A-Za-z]+ [\d]+ [\d]+\:[\d]+\:[\d]+ [A-Za-z]+ [A-Za-z]+\[[\d]+]\: \[[\d]+\:([\d]+)\:[\d]+\] ([A-Za-z0-9 ]+) \[[A-Za-z0-9: ]+\] \[[A-Za-z0-9: ]+\] \{[A-Za-z0-9]+\} ([\d]+\.[\d]+\.[\d]+\.[\d]+)\:[\d]+ \-\> [\d]+\.[\d]+\.[\d]+\.[\d]+\:[\d]+/gm.exec(data);
